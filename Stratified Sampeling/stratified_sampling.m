function [mX,time,std0] = stratified_sampling(magic,wisdom,n_total,percent_pilot,show_results)
std0 = 0;
tstart = tic;
...
for jj = 1:length(s.strata_EX1)
    ...
    weigths_EX1(jj) = get_stratum_weight(wisdom,magic,stratum);
end
...
for jj = 1:length(s.strata_EX0)
    ...
    weigths_EX0(jj) = get_stratum_weight(wisdom,magic,stratum);
end
...
% if round(sum(strata_weigths)+sum(weigths_EX1)+sum(weigths_EX0),12) ~= 1
%     error('Strata-Weights arent zero!')
% end
% 
% if show_results
%     show_strata_results(s,EX_strata,weigths_EX1,weigths_EX0,strata_weigths)
% end
end
...
function show_strata_results(s,EX_strata,weigths_EX1,weigths_EX0,weigths_duo)
STRATUM = s.strata;
MEAN = zeros(length(STRATUM),1);
WEIGTHS = zeros(length(STRATUM),1);
idx_EX1 = contains(s.strata, s.strata_EX1);
idx_EX0 = contains(s.strata, s.strata_EX0);
idx_duo = contains(s.strata, s.strata_duo);
MEAN(idx_EX1) = 1;
MEAN(idx_EX0) = 0;
MEAN(idx_duo) = EX_strata;
WEIGTHS(idx_EX1) = weigths_EX1;
WEIGTHS(idx_EX0) = weigths_EX0;
WEIGTHS(idx_duo) = weigths_duo;
table(STRATUM,MEAN,WEIGTHS)
end